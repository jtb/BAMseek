...
struct BgzfData {
  ...
  static inline short UnpackSignedShort(char* buffer);
...
// unpacks a buffer into an signed short
 inline
   short BgzfData::UnpackSignedShort(char* buffer) {
   union { short value; unsigned char valueBuffer[sizeof(short)]; } un;
   un.value = 0;
   un.valueBuffer[0] = buffer[0];
   un.valueBuffer[1] = buffer[1];
   return un.value;
